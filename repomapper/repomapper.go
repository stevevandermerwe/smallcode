...
func (rm *RepoMapper) extractSymbols(lang *sitter.Language, queryStr string, content []byte) []Symbol {
	...
	// smacker/go-tree-sitter tree doesn't have Close() method in some versions, check its API
	...
	// q.Close() might also be needed if available
...
func (rm *RepoMapper) calculateRanks(files map[string]*FileInfo, root string) {
...
func (rm *RepoMapper) formatTree(root string, files map[string]*FileInfo) string {
